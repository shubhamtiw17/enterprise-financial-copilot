...
async def retrieve_relevant_chunks(
    ...
    """
    Embeds the question, retrieves candidates from pgvector, then reranks.
    """
...
def build_prompt(question: str, chunks: List[dict]) -> List[dict]:
    ...
    Returns plain dicts — each call_* function converts to its own format.
    """
    context_parts = []
...
async def run_rag_query(
    ...
    """
    Full RAG pipeline. The @traceable decorator creates a LangSmith parent run.
    ChatGroq inside call_groq automatically creates a child LLM run.
    """
    from langsmith import traceable

    @traceable(name="rag_pipeline", run_type="chain")
    async def traced_pipeline(q: str):
        ...
        citations = [
            {
                ...
                "score": round(float(chunk["rerank_score"]), 4)
                if "rerank_score" in chunk
                else round(float(chunk["score"]), 4),
            }
            for chunk in chunks
        ]
    ...
    return await traced_pipeline(question)
...
async def call_groq(messages: List[dict]) -> str:
    """
    Uses ChatGroq (langchain-groq) so LangSmith auto-traces the LLM call
    as a child run inside the parent @traceable rag_pipeline run.
    """
